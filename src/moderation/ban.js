...
module.exports = async (interaction) => {
    if (interaction && Object.keys(interaction)) {
        ...
        if (targetUserId === interaction.guild.ownerId) {
            await interaction.editReply(`Hi <@${userWhoMadeRequest}>, you can't ban the owner of the server: <@${targetUserId}>`);
            return;
        }
    
        if (admins.includes(targetUserId)) {
            await interaction.editReply(`Hi <@${userWhoMadeRequest}>, you are not allowed to ban <@${targetUserId}>. Only the owner: <@${process.env.DRAGONIC_ID}> can do that manually.`);
            return;
        }
    
        if (targetUserId === process.env.DEVELOPER_ID) {
            await interaction.editReply("I cannot ban my creator.");
            return;
        }
